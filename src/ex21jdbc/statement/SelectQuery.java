		...
		try
		{
			...
			while(rs.next())
			{
				...
				/*
				 ...
				 형태로 출력된다. 이 경우 date형 자료가 되기 때문에 java.sql.Date클래스의 참조변수로 저장해야한다.
				 */
				 ...
				 */
				String regidate2 = rs.getNString("regidate");
				String regidate3 = rs.getNString("regidate").substring(0, 13);
				String regidate4 = rs.getNString("d1");//별칭을 그대로 사용
				
				System.out.printf("%s %s %s %s %s %s %s\n", id, pw, name, regidate
										, regidate2, regidate3, regidate3);
			}
		}
