...
/*
 내부 정적 클래스
 	...
 	※원래 class앞에는 static을 붙일 수 없으나 내부클래스에서는 허용된다.
 */
class OuterClass{
	
	//인스턴스형 멤버변수
	InnerStaticClass innerStaticClass = new InnerStaticClass();
	static int outerMemberVar;
	
	//인스턴스형 멤버메소드
	static void outerMemberMethod()
	{
	...
	static void outerStaticMethod()
	{
		...
		/*
		 외부클래스의 정적멤버메소드에서는 ㅐㄴ부클래스의 정적멤버만 접근할 수 있다.
...
public class E02InnerStaticClass {
	public static void main(String[] args) {
		...
		/*
		 내부정적 클래스의 정적멤버에 접근할 때는 외부클래스의 객체 생성없이 클래스명으로 바로 접근 가능하다.
		 */
		OuterClass.InnerStaticClass.innerStaticVar = 1;
		OuterClass.InnerStaticClass.innerStaticMethod();
		
		/*
		 내부정적 클래스의 인스턴스형 멤버에 접근할 때는 객체를 생성한 후 참조변수를 통해 접근해야 한다.
		 */
