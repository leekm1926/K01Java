...
/*
 내부 멤버 클래스
 	...
 	-외부클래스의 모든멤버(정적 혹은 인스턴스형)를 사용할 수 있다.
 	-정적멤버를 가질 수 없다. 단 상수는 가능하다.
 	-컴파일시 외부클래스명$내부클래스명.class파일이 생성된다.
 	-이벤트 기반 프로그래밍(윈도우 프로그래밍, 안드로이드 등)에서 주로 많이 사용된다.
 */
...
class OuterMember{
	...
	{
		//외부클래스의 정적메소드에서 내부클래스의 정적 상수에 접근가능
	...
	class InnerMember{
		...
		/*
		 외부클래스는 내부클래스보다 넓은 지역이므로 내부클래스에서는 외부의 모든 멤버를 접근할 수 있다.
		 */
