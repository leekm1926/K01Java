...
public class E03AnonymousClass{
	public static void main(String[] args) {
		...
		/*
		 Student라는 이름을 가진 자식 클래스를 통해 객체를 생성하고 부모의 참조변수로 참조하는 이질화의 형태임.
		 참조변수가 부모타입이므로 자식객체에 직접 접근하는것은 불가능하고 접근시에는 반드시 다운캐스팅(강제형변환)이후
		 접근해야 함.
		 */
		...
		/*
		 ...
		 : Person타입의 인스턴스 변수에person을 상속받은 익명클래스를 생성해서 그 주소를 할당한다.
		...
		Person anonyPerson = new Person("이말년")
		{
			...
			/*
			 ...
			 ->아래멤버는 익명클래스 내에서만 접근이 허용된다.
			 익명이므로 다운캐스팅이 불가능하다.
			 따라서 익명클래스에서는 멤버의 확장을 자주 사용하지 않는다.
			 */
			int newVar;
			void newMethod()
			{
			...
			/*
			 부모타입의 참조변수로 자식객체에서 확장한 멤버에 접근할 수 없으므로 에러가 발생된다.
			 접근하기 위해서는 다운캐스팅이 필요하지만 익명이므로 다운캐스팅이 불가능하다.
			 */
			
	//Student클래스를 상속받아 정의한 익명클래스(즉, Student의 하위객체)
	Student annonyStudent = new Student("김사랑", "2017")
			{
				//익명클래스에서 확장한 멤버변수
				int age = 1;
			...
			annonyStudent.printInfo();
			/*
			 하위 익명클래스에서 확장한 변수이므로 익명객체를 통해서는 접근불가함.
			 */
//			annonyStudent.age = 100;//접근불가 - 에러발생
//			((Student)annonyStudent).age = 100;// 다운캐스팅 불가- 에러발생
		
			/*
			 다운캐스팅은"(자식클래스명)부모타입의 참조변수" 형태로 변환하여 하위클래스의 멤버에 접근하기 위한
			 수단이다. 그러나 익명클래스는 클래스명이 없기 때문에 다운캐스팅 자체가 불가능하다.
			 즉, 익명클래스에서는 새롭게 정의한 멤버에 접근할 수 없다.
			 익명클래스는 오버라이딩의 목적으로만 사용해야 한다.
			 */
			
			/*
			Person anonyTest = new Student("다운캐스팅", "안돼욤")
			{
				String jumin = "001024";
			};
