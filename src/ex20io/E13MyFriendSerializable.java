...
class FriendInfoHandler {
   //멤버변수
   /*
   Friend타입의 객체배열을 서언하여 하위클래스인 High, Univ객체를 모두저장한다.
   부모타입의 배열에는 하위객체 전체를 저장할수 있으므로
   ...
   public void addFriend(int choice) {
      ...
      3.numOfFriends변수를 1 증가시킨다.(후위증가)
       */
      if(choice==1) {
         //고딩친구 입력후 추가
   ...
   3. 객체배열에 저장된 객체들은 for문을 통해 순차적으로 정보출력.
      이때 오버라아딩된 메소드를 통해 항상 해당 객체의 정보를
   ...
   public void deleteInfo() {
      ...
      /*
      배열의 요소중 삭제된 요소의 인덱스값을 저장한 용도의 변수.
      ...
      if(deleteIndex==-1) {
         ...
         //객체배열에서 검색된 요소를 삭제한 후 입력된 위치의 바로뒤 요소를   앞으로 하나씩 교환한다.
   ...
   public void saveFriendInfo() {
      try {
         ...
         //객체배열에 저장된 친구의 갯수만큼 반복하면서 객체를 가져와 순서대로 파일에저장
         for(int i = 0 ; i<numOfFriends ; i++) {
...
public class E13MyFriendSerializable {
   ...
   //프로그램 전반의 흐름에 대해서만 기술하고 관련함수를 호출하는 정도로만 구성.
