...
/*
 ...
 오버로딩 : 함수명은 같으나 매개변수의 갯수, 타입, 순서가 다른 경우를 말한다.
 	즉, 서로 다른 함수이므로 상속받은 하위클래스에서는 오버로딘 된 모든 메소드가 포함된다.
 
 오버라이딩 : 함수명, 매개변수, 반환형까지 모두 동일한 함수를 상속관계에 있는 하위클래스에서 재정의 한것을 말한다.
 	이 경우 하위클래스에서 정의한 메소드가 상위클래스의 메소드를 가리게되므로 항상 최하위 클래스의 메소드가
 	호출되게된다.
...
public class E07RideAndLoad01 {
	...
	ref2.loadMethod();
	ref2.loadMethod(1); // => B클래스의 영역까지만 접근 가능함.
...
//	B refId2 = refId1; // B타입 = A타입 -> 에러발생
//	C refId3 = refId1; // C타입 = A타입 -> 에러발생
	
	//위에서 에러가 난 부분은 아래처럼 다운캐스팅(강제형변환)하면 참조 가능해진다.
	C refId3 = (C) refId1; 
	}
}
