...
/*
 친구를 표현한 최상위 클래스로 해당 프로그램에서는 Friend 클래스로 객체생성은 하지 않는다.
 상속의 목적으로만 정의된 클래스이다.
 */
class Friend{
	//멤버변수
	...
	/*
	 간략한 정보만 출력하는 멤버메소드로 현재 실행부가 없는 상태로 정의되었다.
...
class HighFriend extends Friend{
	...
	/*
	 ...
	 자식에서 확장한 변수를 별도의 print문을 통해 출력하고 있다.
	 */
	@Override
	public void showAllData()
	{
...
public class E10MyFriendInfoBook{
	...
	public static void main(String[] args) {
		...
		연락처정보를 저장할 객체배열의 크기를 100으로 초기화 함.
		*/
		
		
		//무한루프로 구성하여 사용자가 종료하는 시점에 프로그램 종료 됨.
		while(true)
		{
			//메뉴출력을 위한 메소드호출
			menuShow();
...
/*
 프로그램의 흐름을 제어할 목적으로 생성한 클래스로 주로 컨트롤클래스 혹은 핸들러클래스라고 칭한다.
...
class FriendInfoHandler{
	...
	/*
	 ...
	 부모 타입의 배열에는 하위객체 전체를 저장할 수 있으므로 여러개의 객체를 하나의 타입으로 관리할 수 있다는
	 장점이 있다.
	 */
	...
	{
		boolean isFind = false;//검색한 정보가 있는지 확인하기위한 변수
		Scanner scan = new Scanner(System.in);
