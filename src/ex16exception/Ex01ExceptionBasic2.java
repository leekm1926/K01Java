...
/*
 ...
 -main메소드에서 무시하기위해 throws하더라도 오류가 발생되므로 반드시 예외처리를 해줘야한다.
 (무시할 수 없음)
 -예외발생시 JVM은 해당 예외클래스를 인스턴스화해서 프로그램쪽으로 전달하게되고 try~catch에서 받은 후 예외를 처리하게 된다.
 */
public class Ex01ExceptionBasic2 {
	...
	public static void main(String[] args)
		...
		try
		{
			...
			intArr[2] = 300;/*
					예외가 발생하는 지점으로, 예외객체가 전달되므로 아래문장은 실행되지 않음.
					catch블럭으로 바로 넘어가게 됨.
			*/
		...
		catch(ArrayIndexOutOfBoundsException e)
		{
			...
			/*
			 ...
			 e.printStackTrace() : 예외가 발생할 때의 에러메세지를
		...
		/*
		 ...
		 : 숫자를 입력해야되는데 문자열을 잘못 입력하는 경우 발생되는 예외
		 */
		...
		/*
		 ...
		 즉 참조변수만 생성되고, 참조할 객체가 없는 상태에서 사용했을 때 발생되게 된다.
		 */
		...
		try
		{
			System.out.println(toDay.getTime());/*
					toDay 참조변수가 참조할 객체가 없는 상태이므로 해당 메소드를 호출할 수 없어
					예외가 발생된다.
			*/
		}
