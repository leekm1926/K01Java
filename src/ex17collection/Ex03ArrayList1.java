...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		list.add("트와이스");
		list.add(3, "워너원");
		//list.add(5, "소방챀ㅋㅋ");에러발생 (인덱스를 건너뛸 수 없다.)
		...
		/*
		 1-3] 리스트 컬렉션 출력하기(for, 확장for문)
		 	컬렉션을 다룰때는 주로 확장for(for-each)문을 주로 사용하낟.
		 */
		System.out.println("\n[일반for문 사용]");
		...
		/*
		 1-5] 끼워넣기
		 add(삽입할 인덱스, 객체) : 해당 인덱스에 삽입하고 기존데이터는 자동으로 뒤로 밀리게 된다.
		 */
		...
		/*
		 1-6] 포함여부확인
		 	contains(객체) : 찾고자하는 객체가 있는경우 true를 반환한다.
		 				위치한 인덱스까지는 확인할 수 없다.
		 */
		...
		/*
		 1-7] 삭제
		 방법1 : 인덱스로 삭제하기
		 	remove(index) : 인덱스를 통해서 삭제를 진행한 후 삭제가 완료되면 해당 객체를 반환한다.
		 					삭제 후에는 인덱스가 자동으로 재부여된다.
		 					삭제 성공시 해당 객체를 반환하므로 어떤 객체가 삭제되었는지 확인 가능하다.
		 */
